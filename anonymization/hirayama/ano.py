...
def main():
    ...
    if "GENDER" in df.columns:
        df["GENDER"] = mutate_categorical(df["GENDER"], p=0.05)
    if "RACE" in df.columns:
        df["RACE"] = mutate_categorical(df["RACE"], p=0.3)
    if "ETHNICITY" in df.columns:
        df["ETHNICITY"] = mutate_categorical(df["ETHNICITY"], p=0.3)

    # ---- AGE（整数ノイズ; 空欄はそのまま）----
    process_age_add(df, col="AGE", lo=-4, hi=4, min_age=0, max_age=120)
    ...
    process_int_column(df, "num_immunizations",lo=-10,  hi=10)
    ...
    flip_flag_with_prob(df, "depression_flag", p=0.5)
    

    # ---- 実数ノイズ付加 ----
    process_float_add(df, "mean_systolic_bp",   lo=-8.0, hi=16.0, decimals=2)
    process_float_add(df, "mean_diastolic_bp",  lo=-8.0,  hi=16.0,  decimals=2)
