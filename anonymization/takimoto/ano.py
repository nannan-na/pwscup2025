...
def main():
    ...
    if "RACE" in df.columns:
        df["RACE"] = mutate_categorical(df["RACE"], p=0.15)
    if "ETHNICITY" in df.columns:
        df["ETHNICITY"] = mutate_categorical(df["ETHNICITY"], p=0.15)
    ...
    process_int_column(df, "num_immunizations",lo=-2,  hi=2)
    process_int_column(df, "num_devices",      lo=-1,  hi=1)

    # ---- *_flag は確率で反転 ----
    flip_flag_with_prob(df, "depression_flag", p=0.1)

    # ---- 実数ノイズ付加 ----
    process_float_add(df, "mean_systolic_bp",   lo=-5.0, hi=5.0, decimals=2)
    process_float_add(df, "mean_diastolic_bp",  lo=-5.0,  hi=5.0,  decimals=2)

    # ---- 実数ノイズ付加（空欄処理込み) ----
    process_float_with_blanks(df, "mean_bmi",   lo=-3.0,  hi=3.0,  decimals=2)
